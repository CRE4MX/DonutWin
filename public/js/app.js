...
// --- Pure-JS SHA-256 fallback (used when crypto.subtle is unavailable, e.g. plain HTTP) ---
const _sha256 = (function() {
  ...
  function hash(data) {
    ...
    var padLen = ((l + 8) >> 6 << 6) + 64 - l;
    if (padLen < 9) padLen += 64;
    var buf = new Uint8Array(l + padLen);
    buf.set(data);
    buf[l] = 0x80;
...
function _hmacSha256(keyBytes, msgBytes) {
  // keyBytes and msgBytes are Uint8Array
  var blockSize = 64;
  if (keyBytes.length > blockSize) keyBytes = _sha256.hash(keyBytes);
  ...
  var innerHash = _sha256.hash(inner);
  var outer = new Uint8Array(blockSize + 32);
  outer.set(opad); outer.set(innerHash, blockSize);
  return _sha256.hash(outer);
}

const _hasSubtle = typeof crypto !== "undefined" && typeof crypto.subtle !== "undefined" && typeof crypto.subtle.digest === "function";

async function sha256Hex(str) {
  if (_hasSubtle) return toHex(await crypto.subtle.digest("SHA-256", enc.encode(str)));
  return toHex(_sha256.hash(enc.encode(str)));
}
async function hmac256Hex(keyStr, msgStr) {
  ...
  return toHex(_hmacSha256(enc.encode(keyStr), enc.encode(msgStr)));
}
